...
import com.tencent.mps.tie.api.config.RenderPassConfig;
...
public class TsrActivity extends AppCompatActivity implements GLSurfaceView.Renderer, OnFrameAvailableListener {
    ...
    private void checkTsrSdkInit() {
        // 显示加载对话框
        ProgressDialog loadingDialog = new ProgressDialog(this);
        loadingDialog.setMessage("TsrSdk 初始化中...");
        loadingDialog.setCancelable(false);
        loadingDialog.show();

        new Thread(() -> {
            // 循环检查初始化状态和页面状态
            while (TsrSdkHelper.getInstance().isInit() == null && !isDestroyed()) {
                try {
                    Thread.sleep(100); // 每100ms检查一次，降低CPU消耗
                } catch (InterruptedException e) {
            ...
            runOnUiThread(() -> {
                ...
                if (TsrSdkHelper.getInstance().isInit()) {
                    init();
                } else {
                    Toast.makeText(this, "TsrSdk Init Fail", Toast.LENGTH_LONG).show();
                    finish(); // 关闭当前页面
                }
            });
        }).start();
    ...
    private void init() {
        ...
        if (mAlgorithm == Algorithm.SR_STD) {
            ...
            config.put(RenderPassConfig.ALGORITHM_TYPE, TSRPass.TSRAlgorithmType.STANDARD);
            mTSRPass = new TSRPass(config);
        } else if (mAlgorithm == Algorithm.SR_STD_EH) {
            Map<String, Object> config = new HashMap<>();
            config.put(RenderPassConfig.ALGORITHM_TYPE, TSRPass.TSRAlgorithmType.STANDARD_COLOR_RETOUCHING_EXT);
            mTSRPass = new TSRPass(config);
            ...
            Map<String, Object> config = new HashMap<>();
            config.put(RenderPassConfig.ALGORITHM_TYPE, TIEPass.TIEAlgorithmType.STANDARD);
            mTIEPass = new TIEPass(config);
        ...
        if (mCompareAlgorithm == Algorithm.SR_STD) {
            ...
            Map<String, Object> config = new HashMap<>();
            config.put(RenderPassConfig.ALGORITHM_TYPE, TIEPass.TIEAlgorithmType.STANDARD);
            mTIEPassCmp = new TIEPass(config);
        ...
        initView(mIsRecordVideo, mAlgorithm, mCompareAlgorithm);
    }
...
//            Log.i(TAG, "reinit cost = " + (end - start));
            ...
            switch (error) {
                ...
                case ML_MODEL_NOT_CONFIGURE:
                    ...
                    runOnUiThread(() -> DialogUtils.showSimpleConfirmDialog(mContext, String.format(finalErrorMsg),
                            (dialogInterface, i) -> {
                            }));
                    break;
                default:
                    runOnUiThread(() -> DialogUtils.showSimpleConfirmDialog(mContext, "Unknown error",
                            (dialogInterface, i) -> {
                            }));
            }
        ...
        if (!mHasValidFrame) {
            ...
            switch (mAlgorithm) {
                ...
                case SR_PRO_EH:
                    ...
                    mProcessedTexture2D = mTSRPass.render(mInputTexture2D);
                ...
                case IE_STD:
                    ...
                    mProcessedTexture2D = mTIEPass.render(mInputTexture2D);
                    break;
            }
        }
        ...
        if (mMediaRecorder != null) {
            ...
            switch (mCompareAlgorithm) {
                ...
                case SR_PRO_EH:
                    ...
                    cmpTextureId = mTSRPassCmp.render(mInputTexture2D);
                    mCompareTexDrawer.draw(mProcessedTexture2D, cmpTextureId);
                    break;
                case IE_PRO:
                case IE_STD:
                    ...
                    cmpTextureId = mTIEPassCmp.render(mInputTexture2D);
                    mCompareTexDrawer.draw(mProcessedTexture2D, cmpTextureId);
                    break;
            }
        }
    ...
    private boolean isSrAlgorithm(Algorithm algorithm) {
        return algorithm == Algorithm.SR_STD || algorithm == Algorithm.SR_STD_EH || algorithm == Algorithm.SR_PRO
                || algorithm == Algorithm.SR_PRO_EH;
    }
