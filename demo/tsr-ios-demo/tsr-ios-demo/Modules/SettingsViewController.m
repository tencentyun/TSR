...
@property (strong, nonatomic) UIScrollView *scrollView; // 添加滚动视图
@property (strong, nonatomic) UIView *contentView; // 内容容器视图
...
- (void)viewDidLoad {
    ...
    self.chooseTypeSegmentedControl.frame = CGRectMake(left, top, viewWidth - 40, 50);
    ...
    [self.contentView addSubview:self.chooseTypeSegmentedControl];
    
    // 添加用于显示视频名称的标签
    top += 50;
    self.selectedVideoURL = [[NSBundle mainBundle] URLForResource:@"480x854" withExtension:@"mp4"];
    self.videoNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(left, top, viewWidth - 40, 50)];
    self.videoNameLabel.text = @"选中视频: 480x854";
    [self.contentView addSubview:self.videoNameLabel];
    
    // 初始化数据
    self.data = @[@"480x854", @"720x1280", @"854x480", @"1280x720", @"1920x1080"];
    ...
    // 创建并设置UICollectionView（添加到主视图而非 contentView，因为它是覆盖层）
    ...
    self.showCollectionViewButton.frame = CGRectMake(left, top, viewWidth - 40, 50);
    ...
    [self.contentView addSubview:self.showCollectionViewButton];
    ...
    self.chooseFileButton.frame = CGRectMake(left, top, viewWidth - 40, 50);
    self.chooseFileButton.hidden = YES;
    [self.contentView addSubview:self.chooseFileButton];
    
    // 分割线
    top += 50;
    UIView *line1 = [[UIView alloc] initWithFrame:CGRectMake(0, top, viewWidth, 1)];
    line1.backgroundColor = [UIColor grayColor];
    [self.contentView addSubview:line1];
    
    // 选择算法标签
    _selectAlgorithmLabel = [[UILabel alloc] initWithFrame:CGRectMake(left, top, viewWidth - 40, 50)];
    _selectAlgorithmLabel.text = @"选择算法";
    [self.contentView addSubview:_selectAlgorithmLabel];
    ...
    // 创建 UIPickerView
    top += 50;
    self.algorithmPickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(left, top, viewWidth - 40, 100)];
    self.algorithmPickerView.dataSource = self;
    self.algorithmPickerView.delegate = self;
    [self.contentView addSubview:self.algorithmPickerView];
    
    top += 120;
    UIView *line2 = [[UIView alloc] initWithFrame:CGRectMake(0, top, viewWidth, 1)];
    line2.backgroundColor = [UIColor grayColor];
    [self.contentView addSubview:line2];
    
    // 超分辨率标签
    _srLabel = [[UILabel alloc] initWithFrame:CGRectMake(left, top, viewWidth - 40, 50)];
    _srLabel.text = @"放大倍数";
    [self.contentView addSubview:_srLabel];
    ...
    self.resolutionRatioControl.frame = CGRectMake(left, top, viewWidth - 40, 50);
    self.resolutionRatioControl.selectedSegmentIndex = 5; // 默认选择auto
    [self.contentView addSubview:self.resolutionRatioControl];
    
    // 分割线
    top += 70;
    UIView *line3 = [[UIView alloc] initWithFrame:CGRectMake(0, top, viewWidth, 1)];
    ...
    self.startPlayButton.frame = CGRectMake(viewWidth / 2 - 50, top, 100, 50);
    [self.contentView addSubview:self.startPlayButton];
    ...
    self.startExportButton.frame = CGRectMake(viewWidth / 2 - 50, top, 100, 50);
    [self.contentView addSubview:self.startExportButton];
    
    // 设置内容视图的大小（添加底部边距）
    top += 100;
    self.contentView.frame = CGRectMake(0, 0, viewWidth, top);
    self.scrollView.contentSize = CGSizeMake(viewWidth, top);
    
    // 创建并设置覆盖其他控件的视图（添加到主视图）
