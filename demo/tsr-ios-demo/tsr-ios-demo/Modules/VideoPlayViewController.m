...
- (instancetype)initWithVideoURL:(NSURL *)videoURL srRatio:(float)srRatio algorithm:(NSString*)algorithm isRecording:(BOOL)isRecording {
    ...
    if (self) {
        ...
        if (srRatio > 0) {
            ...
            rect = CGRectMake(0, 0, _videoSize.width * srRatio, _videoSize.height * srRatio);
        } else {
            ...
            rect = CGRectMake(0, 0, viewWidth, viewHeight);
        }
        ...
        if (_isRecording) {
            dispatch_async(_serialQueue, ^{
                ...
                self->_lastFrameTime = kCMTimeZero;
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    ...
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        ...
        if (self->_srRatio > 0) {
            ...
            // 计算适配屏幕的显示尺寸（保持宽高比）
            double outputRatio = 1.0 * outputWidth / outputHeight;
            double screenRatio = 1.0 * screenWidth / screenHeight;
            ...
            if (screenRatio > outputRatio) {
                ...
                displayWidth = (int)(displayHeight * outputRatio);
            } else {
                // 屏幕更高，以宽度为准
                displayWidth = screenWidth;
                displayHeight = (int)(displayWidth / outputRatio);
            }
            
            rect = CGRectMake(0, 0, displayWidth, displayHeight);
            ...
            if (screenRatio > videoRatio) {
                ...
                // 屏幕更高，以宽度为准
                viewWidth = screenWidth;
                viewHeight = (int)(viewWidth / videoRatio);
            }
