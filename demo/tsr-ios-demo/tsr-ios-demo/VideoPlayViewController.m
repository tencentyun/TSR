...
- (void)verifyTSRLicenseAndCreateSRPass:(CGSize)videoSize srRatio:(float)srRatio device:(id<MTLDevice>)device{
    ...
    TSRSdkLicenseStatus status = [TSRSdk.getInstance initWithAppId:APPID licenseURL:resourcesURL tsrLogger:[[Logger alloc] init]];
    if (status == TSRSdkLicenseStatusAvailable) {
        [self.infoLabel setText:@"TSR: ON"];
        _isTsrOn = true;

        int outputWidth = videoSize.width * srRatio;
        int outputHeight = videoSize.height * srRatio;
        NSLog(@"inputWidth = %d, inputHeight = %d, srRatio = %f, outputWidth = %d, outputHeight = %d",
              (int)videoSize.width, (int)videoSize.height, srRatio, outputWidth, outputHeight);
        ...
        _sr_texture = [device newTextureWithDescriptor:textureDescriptor];

        // 初始化tsrpass
        _tsr_pass = [[TSRPass alloc] initWithDevice:device inputWidth:videoSize.width inputHeight:videoSize.height srRatio:srRatio];
    } else {
...
- (instancetype)initWithVideoURL:(NSURL *)videoURL srRatio:(float)srRatio {
    ...
    if (self) {
        ...
        CGSize videoSize = [self loadVideo:videoURL];
        MTLTextureDescriptor *textureDescriptor = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatBGRA8Unorm width:videoSize.width height:videoSize.height mipmapped:NO];
        ...
        if (srRatio > 0) {
            ...
            _isTsrOn = true;
            [self.infoLabel setText:@"TSR: ON"];
            [self verifyTSRLicenseAndCreateSRPass:videoSize srRatio:srRatio device:device];
        } else {
            _isUseTsr = false;
            _isTsrOn = false;
            [self.infoLabel setText:@"TSR: OFF"];
