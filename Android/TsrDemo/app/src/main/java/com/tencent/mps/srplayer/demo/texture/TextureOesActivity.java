...
/**
 * OES 纹理路径 Demo：演示如何把 MediaCodec 的 Surface 输出（OES 纹理）直接交给
 * {@link TieTextureEnhancer} 做画质增强并上屏，是最简集成路径。
 *
 * <p>管线：MediaCodec → SurfaceTexture(OES) → TieTextureEnhancer.process() → 屏幕。</p>
 */
public class TextureOesActivity extends AppCompatActivity implements GLSurfaceView.Renderer {

    private static final String TAG = "TextureOesActivity";
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        // 1) 构造 TieTextureEnhancer（OES 输入 + 上屏直出），引擎异步初始化
        //    色彩标准按 codec VUI 探测结果传入，读不到时按分辨率启发式
        ...
        mOesTextureId = mTieTextureEnhancer.init(this, cfg, result -> {
            runOnUiThread(() -> {
                ...
                if (!canEnhance) {
                    ...
                    Log.i(TAG, "ready: " + result.outputWidth + "x" + result.outputHeight);
                }
                refreshStatusText();
            });
        });

        // 2) SurfaceTexture → Surface → 解码器
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        // 暂停时：跳过解码器出队和 updateTexImage，复用 SurfaceTexture 中保留的最后一帧 OES 纹理

        // 按视频纵横比设置居中视口，避免拉伸
        applyCenteredViewport();

        boolean useEnhance = mToggleEnhanceOn && canEnhance() && mTieTextureEnhancer != null;
        if (useEnhance) {
            // SDK TieTextureEnhancer：OES → 增强 → 上屏
            long tProcess = SystemClock.elapsedRealtime();
            mTieTextureEnhancer.process(mOesTextureId, GLES11Ext.GL_TEXTURE_EXTERNAL_OES, mStMatrix);
    ...
    private String buildStaticText() {
        ...
        if (mInitResult != null && mInitResult.path == TieTextureEnhancer.Path.SHADER) {
            return "OES | 使用 SHADER";
        }
        return String.format(Locale.getDefault(),
                "OES→RTS | video=%dx%d | view=%dx%d | ready=%s",
    ...
    private String buildPerfText() {
        ...
        String pathStr = canEnhance() ? mInitResult.path.name() : "NONE";
