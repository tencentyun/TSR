...
public class Decode2Surface {
    ...
    /** KEY_COLOR_RANGE 值，读不到时为 -1；用于上游 {@link ColorSpaceUtil} 推断 ColorStandard */
    private int mColorRange = -1;
    /** KEY_COLOR_STANDARD 值，读不到时为 -1；同上 */
    ...
    private void initInternal(MediaExtractor extractor, String sourceDesc, Surface surface) throws IOException {
        ...
        mColorRange = readOptionalColorInt(format, MediaFormat.KEY_COLOR_RANGE);
        mColorStandard = readOptionalColorInt(format, MediaFormat.KEY_COLOR_STANDARD);

        Log.i(TAG, "init() video: " + mWidth + "x" + mHeight + " mime=" + mime + " duration=" + mDurationUs + "us"
                + " colorRange=" + mColorRange + " colorStandard=" + mColorStandard);
    ...
    /** {@link MediaFormat#KEY_COLOR_RANGE} 值；codec 未写时为 -1。供 {@link ColorSpaceUtil} 推断用。 */
    public int getColorRange() {
        return mColorRange;
    }

    /** {@link MediaFormat#KEY_COLOR_STANDARD} 值；codec 未写时为 -1。供 {@link ColorSpaceUtil} 推断用。 */
    ...
    /** 读 KEY_COLOR_* 这类可选整型字段；不存在或读取失败时返回 -1。 */
    private static int readOptionalColorInt(MediaFormat format, String key) {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N
                && format.containsKey(key)) {
            try {
                return format.getInteger(key);
            } catch (Exception e) {
                Log.w(TAG, "read " + key + " failed: " + e.getMessage());
            }
        }
        return -1;
    }
