...
import com.tencent.mps.srplayer.common.video.Decode2Yuv;
 ...
 * </ul>
 */
public class ByteBufferBaseActivity extends AppCompatActivity implements GLSurfaceView.Renderer {

    private static final String TAG = "ByteBufferBaseActivity";
    ...
    /// 性能统计与帧率调度
    /** 渲染性能仪表（fps + SDK 耗时，GL 线程写、UI 线程读）。状态栏展示以此为准。 */
    protected final PassTimingMeter mTimingMeter = new PassTimingMeter();
    /** SDK init 失败详情（null=成功）。子类通过 {@link #onInitFailed} 设置。 */
    protected volatile String mInitFailMsg = null;
    ...
    protected volatile boolean mToggleButtonOn = false;
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        //     这次渲染会把最后一帧上屏，但不计入统计（避免污染暂停瞬间的统计）。
        ...
        // 注意：dequeue 失败/yuvData==null 等早返回路径不计入帧耗时。
        ...
        if (!mRendererInitialized) {
            ...
            Nv12Shaders.YuvColorSpace cs = ColorSpaceUtil.toNv12(
                    ColorSpaceUtil.detectForSdk(
                            mDecoder.getColorRange(),
                            mDecoder.getColorStandard(),
                            videoW));
        ...
        // 暂停尾帧分支：渲染 + 缓存已完成，跳过后续统计与调度，避免污染性能统计。
        if (cachingTailFrame) {
            return;
        }

        // 节流刷新性能指标到 tvStatus1（fps + 耗时），约 1s 一次
     ...
     * 子类可在 init 失败时调 {@link #onInitFailed} 后再调本方法。
     */
    ...
    /**
     * 子类调用：记录 SDK init 失败详情，并刷新 tvStatus2。
     * @param msg 失败详情（code + msg）；传 null 表示成功（清空）
     */
    protected void onInitFailed(String msg) {
        mInitFailMsg = msg;
        refreshStatusText();
    }

    /**
     * tvStatus2：静态信息。init 失败时优先显示失败详情。
     * 子类可 override 追加自身静态信息，但应保留 init 失败的优先显示逻辑。
     */
    protected String buildStaticText() {
        if (mInitFailMsg != null) {
            return "init 失败 | " + mInitFailMsg;
        }
        return String.format(Locale.getDefault(),
                "ByteBuffer | video=%dx%d | view=%dx%d",
                mVideoWidth, mVideoHeight, mViewWidth, mViewHeight);
    }

    /**
     * tvStatus1：动态性能指标（fps + SDK 耗时，四舍五入到整数）。
     * 子类可 override 追加自身性能信息（如路径）。
     */
    protected String buildPerfText() {
    ...
    /**
     * 暂停期间的预览渲染：复用缓存的 NV12 数据按当前 mEnhance 状态走一遍 process + renderFrame，不累计性能统计。
     *
