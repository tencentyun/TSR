 ...
 *
 * <p>正向 RGB&rarr;YUV（{@link OesYuvSplitter} 写出 Y/UV）与反向 YUV&rarr;RGB
 * （{@link Nv12GpuRenderer} 的 GPU-UV 路径、{@link Nv12Renderer} 的 CPU-UV 路径上屏）
 * 共用同一份色彩约定（默认 BT.601 limited），shader 改一处即全部生效，
 * 避免"多条路径色彩公式不一致"这类漏改 bug。</p>
 */
public final class Nv12Shaders {
     ...
     *
     * <p>系数严格与 {@link OesYuvSplitter} 的正向 RGB&rarr;YUV 配对（OES 路径只用 BT.601 limited）。</p>
     */
    public enum YuvColorSpace {
        /** 标清/兼容兜底；与 OesYuvSplitter 正向公式严格配对。 */
        BT601_LIMITED,
        /** 部分手游录屏；Y/UV 都不做 16/255 拉伸。 */
        BT601_FULL,
        /** 720p/1080p 主流视频；现代云游戏推流一般属于此档。 */
        BT709_LIMITED,
        /** 极少出现，仅做完整性兜底。 */
        BT709_FULL,
    }
    ...
    // ---- GLES30 版本 shader/program 工具（供 TsrPass 管线等 GLES 3.0+ 组件复用） ----
