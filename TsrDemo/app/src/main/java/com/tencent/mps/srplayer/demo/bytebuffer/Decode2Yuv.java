...
public class Decode2Yuv {
    ...
    // 色彩范围 / 色域。API 24+ 才能读；codec 未写 VUI 时可能读不到。
    // 为别：-1 = 不可用，其他值取自 {@code MediaFormat.KEY_COLOR_RANGE} / {@code KEY_COLOR_STANDARD}。
    ...
    public void init(String filePath) throws IOException {
        ...
        initInternal(extractor, filePath);
    }
     ...
     */
    public void init(Context context, Uri uri) throws IOException {
        MediaExtractor extractor = new MediaExtractor();
        extractor.setDataSource(context, uri, null);
        initInternal(extractor, uri.toString());
    }

    private void initInternal(MediaExtractor extractor, String sourceDesc) throws IOException {
    ...
    /**
     * 获取色彩范围（{@code MediaFormat.KEY_COLOR_RANGE}），API 24+ 才能拿到；
     * codec 未写 VUI / 低于该 API 时返回 -1。常见取值：
     * {@code MediaFormat.COLOR_RANGE_LIMITED}=2、{@code COLOR_RANGE_FULL}=1。
     */
    ...
    /**
     * 获取色域标准（{@code MediaFormat.KEY_COLOR_STANDARD}），API 24+ 才能拿到；
     * codec 未写时返回 -1。常见取值：
     * BT601_PAL=2、BT601_NTSC=4、BT709=1、BT2020=6。
     */
    ...
    private void updateOutputFormat(MediaFormat format) {
        ...
        // 读取色彩 range / 色域。这两个 key 在 API 24 引入，与上面字段同为可选，
        // 部分 codec 不会写。读不到保持 -1，由上层决定是否需要启发式推断。
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            try {
                if (format.containsKey(MediaFormat.KEY_COLOR_RANGE)) {
                    mColorRange = format.getInteger(MediaFormat.KEY_COLOR_RANGE);
                }
            } catch (Exception e) {
                Log.w(TAG, "updateOutputFormat() read KEY_COLOR_RANGE failed: " + e.getMessage());
            }
            try {
                if (format.containsKey(MediaFormat.KEY_COLOR_STANDARD)) {
                    mColorStandard = format.getInteger(MediaFormat.KEY_COLOR_STANDARD);
                }
            } catch (Exception e) {
                Log.w(TAG, "updateOutputFormat() read KEY_COLOR_STANDARD failed: " + e.getMessage());
            }
        }
