...
The TSRSdk package encompasses two primary methods: ``initWithAppId:licenseUrl:tsrLogger:`` and ``deInit``. The ``initWithAppId:licenseUrl:tsrLogger:`` method is designated for initializing the SDK, while the ``deInit`` method is employed for the liberation of resources.


1. To start TSRSdk for offline use, provide an APPID and LICENSE_PATH. Also, include a TSRLogger for log retrieval. Example code:
