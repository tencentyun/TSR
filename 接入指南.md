...
// Here we set thess parameters to slightly enhance the image.
tsrPass.setParameters(51, 51, 55);
...
TSRPass是用于进行超分辨率渲染的类，它包括了init、setParameters、render和release方法。
...
```
 ...
 [_tsr_pass applyColorAdjustmentWithBrightness:52 saturation:52 contrast:58];
```
